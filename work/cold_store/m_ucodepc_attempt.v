...
//            wire       check_rs1_rd = (opcode[5:4] == 2'b11) && (funct3[1:0] == 2'b00);
            ...
            // =======================================================
            // HIGLEVEL, Check on most codespaces, omits some minor codespaces
            // =======================================================
            ...
            wire         qualint_or_RST_I = qualint | RST_I;
            wire         illegal_or_qualint_AND_corerunning = (illegal | qualint) & corerunning;
            wire         illegal_or_qualint_or_RST_I_AND_corerunning = (illegal | qualint | RST_I) & corerunning;
            assign takebranch = 
                                (Adr0Mustbe0 & B[0]) |
                                (Adr1Mustbe0 & B[1]) |                
                                (use_brcond & is_brcond) |
            ...
            assign minx[0]   = corerunning & (buserror | (usedinx_or_RST_I ? (illegal_or_qualint_or_RST_I_AND_corerunning ? qualint_or_RST_I : dinx[0]) : (maybranch ? takebranch : rinx[0])));
            ...
            assign ucodepc_killwarnings = INSTR[31] | &INSTR[29:15] | &INSTR[11:7] | &INSTR[1:0] | &B;
            assign illegal = 1'b0;
         end                 
...
//          wire       funct7_5_dontcare = (opcode[5:3] == 3'b010 && funct3 == 3'b101) || (opcode[5:3] == 3'b110 && (funct3 == 3'b101 || funct3 == 3'b000));            
            ...
            assign ucodepc_killwarnings = INSTR[31] | &INSTR[29:15] | &INSTR[11:7] | &B | &INSTR;

         end else if ( LAZY_DECODE == 1 ) begin

            // =======================================================
            // LOWLEVEL: Check on most codespaces, omits some minor codespaces
            // =======================================================
            ...
            SB_LUT4 #(.LUT_INIT(16'he4ff)) illegal_b_cmb( .O(illegal_b), .I3(INSTR[5]), .I2(INSTR[4]), .I1(INSTR[3]), .I0(INSTR[2]));
            wire illegal_c;
            SB_LUT4 #(.LUT_INIT(16'h7777)) illegal_c_cmb( .O(illegal_c), .I3(1'b0), .I2(1'b0), .I1(INSTR[1]), .I0(INSTR[0]));
            SB_LUT4 #(.LUT_INIT(16'hffca)) illegal_cmb(.O(illegal), .I3(illegal_c), .I2(INSTR[6]), .I1(illegal_b), .I0(illegal_a));

            wire         illegal_or_qualint_or_RST_I_AND_corerunning;
            SB_LUT4 #(.LUT_INIT(16'hfe00)) cmb_bbb(.O(illegal_or_qualint_or_RST_I_AND_corerunning), .I3(corerunning), .I2(qualint), .I1(illegal), .I0(RST_I));
//            assign illegal_or_qualint_or_RST_I_AND_corerunning = (illegal | qualint | RST_I) & corerunning;
             ...
             * B[31] -------------|_|
             */
            //wire         takebranch = (Adr0Mustbe0 & B[0]) | (Adr1Mustbe0 & B[1]) | (use_brcond & is_brcond) | ((sa32 & ~sa15) & B[31] );
            wire         sa32_nsa15;
            wire         takebranch_a, takebranch_b;
            SB_LUT4 #(.LUT_INIT(16'h4444)) l_sa32_nsa15(.O(sa32_nsa15), .I3(1'b0), .I2(1'b0), .I1(sa32), .I0(sa15));
            SB_LUT4 #(.LUT_INIT(16'hf888)) takebranch_a_cmb( .O(takebranch_a), .I3(Adr0Mustbe0), .I2(B[0]), .I1(Adr1Mustbe0), .I0(B[1]));
             ...
             * assign minx[0]   = corerunning & (buserror | (usedinx_or_RST_I ? (illegal_or_qualint_or_RST_I_AND_corerunning ? qualint_or_RST_I : dinx[0]) : (maybranch ? takebranch : rinx[0])));
             * What I want:
             * buserror == 1 -> minx[0] = 1                            This can only happen when we are waiting for ACK from I/O or SRAM
             * RST_I    == 1 -> minx[0] = 1                            but only if also core is running
             * usedinx  == 1 : illegal = 1 -> minx[0] = 0              but only if also core is running 
             * usedinx  == 1 : qualint == 1 -> minx[0] = 1             but only if also core is running 
             *                 qualint == 0 -> minx[0] = dinx[0]       but only if also core is running 
             * usedinx  == 0 : maybranch == 1 -> minx[0] = takebranch  but only if also core is running 
             *                 maybranch == 0 -> minx[0] = rinx[0]     but only if also core is running 
             * 
             * minx0b = maybranch ? (takebranch_a | takebranch_b) : rinx[0]; 
             * dinx0_and_legal = dinx[0] & ~illegal;
             * minx0a = usedinx ? (qualint | dinx0_and_legal ) : minx0b;
             * minx[0] = corerunning & ( RST_I | buserror | minx0a);
             */         
            wire         minx0a, minx0b;
            wire         dinx0_and_legal;
            SB_LUT4 #(.LUT_INIT(16'h0f00)) dinx0_and_legal_cmb( .O(dinx0_and_legal), .I3(dinx[0]), .I2(illegal), .I1(1'b0), .I0(1'b0));
            ...
            /* wire         illegal_or_qualint_AND_corerunning = (illegal | qualint) & corerunning;
             * assign minx[1]   = usedinx_or_RST_I ? ( (dinx[1] | illegal_or_qualint_AND_corerunning) & ~RST_I)     : rinx[1];
             *
             * usedinx
             * |RST_I
             * ||illegal_or_qualint_AND_corerunning .. uses illegal_or_qualint_or_RST_I_AND_corerunning instead
             * |||dinx[1]
             * ||||rinx[1]
             * |||||       minx[1]
             * 00xx0       0
            ...
            // =======================================================
            // LOWLEVEL: No error checking at all
            // =======================================================

            wire         illegal_or_qualint_or_RST_I_AND_corerunning;
//          assign illegal_or_qualint_or_RST_I_AND_corerunning = (illegal | qualint | RST_I) & corerunning;
            SB_LUT4 #(.LUT_INIT(16'hfe00)) cmb_bbb(.O(illegal_or_qualint_or_RST_I_AND_corerunning), .I3(corerunning), .I2(qualint), .I1(1'b0), .I0(RST_I));
            //wire         takebranch = (Adr0Mustbe0 & B[0]) | (Adr1Mustbe0 & B[1]) | (use_brcond & is_brcond) | ((sa32 & ~sa15) & B[31] );
            wire         sa32_nsa15;
            wire         takebranch_a, takebranch_b;
            SB_LUT4 #(.LUT_INIT(16'h4444)) l_sa32_nsa15(.O(sa32_nsa15), .I3(1'b0), .I2(1'b0), .I1(sa32), .I0(sa15));
            ...
            wire         minx0a, minx0b;
            SB_LUT4 #(.LUT_INIT(16'hfcaa)) minx0b_cmb( .O(minx0b), .I3(maybranch), .I2(takebranch_a), .I1(takebranch_b), .I0(rinx[0]));
            SB_LUT4 #(.LUT_INIT(16'hfcaa)) minx0a_cmb( .O(minx0a), .I3(usedinx_or_RST_I), .I2(qualint), .I1(dinx[0]), .I0(minx0b));
            SB_LUT4 #(.LUT_INIT(16'hfe00)) minx_cmb0( .O(minx[0]), .I3(corerunning), .I2(RST_I), .I1(buserror), .I0(minx0a));
