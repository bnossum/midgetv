...
typedef struct {
        ...
        uint64_t pairconstraint;
        uint64_t considered_columns;
        uint64_t inputs_for_column[NRCOLUMNS];
...
#define BIGTEXTSIZE (1000*1024)
char thetabletext1[256][LINEBUFFSIZE];
char thetabletext2[256][LINEBUFFSIZE];
char *preamble;
char *postamble;
void prepare_for_output( void ) {
        ...
        if ( (postamble = malloc( BIGTEXTSIZE ))== NULL )
                ferr( "Que?\n" );
        if ( (preamble = malloc( BIGTEXTSIZE ))== NULL )
                ferr( "Que?\n" );
        p = preamble;
        
        if ( (fi = fopen(fname,"r")) == NULL )
                ferr( "Can't find specification file %s\n", fname );
        do {
                if ( (q=fgets(linebuffer,LINEBUFFSIZE,fi)) != NULL ) {
                        switch ( state ) {
                        case 0 :
                                if ( strstr( linebuffer, "//ORIGTAB") ) {
                                        state = 1;
                                } 
                                strcpy( p, linebuffer );
                                p += strlen( linebuffer );
                                if ( strlen( preamble ) >= BIGTEXTSIZE - LINEBUFFSIZE )
                                        ferr( "Que\n" );
                                break;
                        case 1 :
                                if ( strstr( linebuffer+5," */Y( ") == NULL )
                                        ferr( "Format error in clumsy parsing, table line %d\b", ti );
                                if ( linebuffer[20] != ',' )
                                        ferr( "Format error in clumsy parsing, table line %d\b", ti );
                                if ( linebuffer[41] != ',' )
                                        ferr( "Format error in clumsy parsing, table line %d\b", ti );
                                linebuffer[21] = 0;
                                strcpy(thetabletext1[ti],linebuffer);
                                strcpy(thetabletext2[ti], linebuffer+41);
                                ti++;
                                if ( ti >= 256 )
                                        state = 2;
                                p = postamble;
                        ...
                        case 2 :
                                strcpy( p, linebuffer );
                                p += strlen( linebuffer );
                                if ( strlen( postamble ) >= BIGTEXTSIZE - LINEBUFFSIZE )
                                        ferr( "Que\n" );
                                break;                                
                        }
...
TBL *initiate( void ) {
        ...
        */
        ...
        uint64_t considered_columns = 0;

        // We do not consider columns that take part of dinx
        for ( c = 0; c < NRCOLUMNS-8; c++ ) {
                isolate_column_information(tblp, c, value, valid);
...
//                }
        ...
        for ( c = 0; c < NRCOLUMNS; c++ ) {
                ...
                printf( ". %3d zeros, %3d ones, %3d don't cares.", tblp->nr_zero_in_column[c], tblp->nr_ones_in_column[c],
                        256-tblp->nr_zero_in_column[c]-tblp->nr_ones_in_column[c]);
                if ( tblp->pairconstraint & (1ull << c) ) {
                        printf( " Pair " );
                } else {
                        if (considered_columns & (1ull<<c) ) {
                                printf( "      " );
                ...
                if ( (considered_columns & (1ull<<c) ) == 0 ) {
...
/////////////////////////////////////////////////////////////////////////////
int evaluatetwo( TBL *tblp, int inxa, int inxb, int wantassolution ) {
...
/////////////////////////////////////////////////////////////////////////////
int evaluate_aggregate( TBL *tblp, int inx, int wantnreq, uint64_t considered_columns ) {
        int i;
        int tbl[256];
...
//                                        ferr( "Internal\n" );
        ...
        static uint32_t max_lost_dontcares = ~0u;
        int c;
        if ( possibilities > 0 ) {
                ...
                if ( nreq >= wantnreq ) {                        
                        ...
                        // How many don't care does this solution cost us?
                        uint32_t nrdontcares = 0;
                        for ( c = 0; c < NRCOLUMNS; c++ ) 
                                if ( (considered_columns >> c) & 1 ) 
                                        nrdontcares = nrdontcares + 256 - tblp->nr_zero_in_column[c] - tblp->nr_ones_in_column[c];
                        if ( nrdontcares > max_lost_dontcares ) {
                                printf( "Solution, but loses more don't cares (%d)\n", nrdontcares );
                        } else {
                                max_lost_dontcares = nrdontcares;
                                printf( "Solution, nr lost don't cares: %d", nrdontcares );
                                ...
                                // Copy to solution
                                sol.nrdontcares = nrdontcares;
                                for ( i = 0; i < 256; i++ ) 
                                        sol.tbl[i] = tbl[i];
                                for ( i = 0; i < NRCOLUMNS; i++ ) {
                                        sol.perm[i] = aga->perm[i];
                                        sol.func[i] = aga->func[i];
                                }                                                                                
                        }
                }
        }
...
/////////////////////////////////////////////////////////////////////////////
int findevaluationorder( int evaluationorder[], uint64_t selected, TBL *tblp ) {
        int i;
        int start = -1;
...
/////////////////////////////////////////////////////////////////////////////
void give_output( void ) {
        int i;
        
        // Print solution
//        printf( "Solution, nr lost don't cares: %d)n", sol.nrdontcares );
//        for ( i = 0; i < 256; i++ ) {
//                if ( i == sol.tbl[i] ) {
//                        printf( "        0x%3.3x,", i );
//                } else {
//                        printf( "/*%2.2x<-*/0x%3.3x,", i, sol.tbl[i] & 0xfffu);
//                }
//                printf( "\n" );
//        }
//        for ( i = 0; i < NRCOLUMNS; i++ )
//                if ( sol.perm[i] != 0 )
//                        printf( "%2d 0x%2.2x 0x%4.4x\n", i, sol.perm[i], sol.func[i] );
//
        printf( "// This file generated from %s, modified by %s. Do not edit.\n", fname, __FILE__ );
        printf( "%s", preamble );
        for ( i = 0; i < 256; i++ ) {
                printf( "%s", thetabletext1[sol.tbl[i]] );

                printf( "[fitte]" );
                
                printf( "%s", thetabletext2[sol.tbl[i]] );
        }
...
//        for ( NrColsToRecode = __builtin_popcount(tblp->considered_columns); NrColsToRecode > 2; NrColsToRecode-- ) { // With many degrees of freedom this works well. But my table is nearly full
        for ( NrColsToRecode = 2; NrColsToRecode > 0; NrColsToRecode-- ) {
                selected = 0;
                while ( (selected = clumsy_nextperm( selected, tblp->considered_columns, NrColsToRecode) ) != 0) {
                        ...
                        VECTOR3PRI( (uint32_t *)&selected, (const uint32_t *)&(tblp->considered_columns), NRCOLUMNS);
                        printf( " " );
                        fflush( stdout );
        

                        if ( evaluatetwo( tblp, evaluationorder[start], evaluationorder[start+1], NrColsToRecode < 3 )  == 0 ) {
                        ...
                        for ( i = start+2; i < NRCOLUMNS; i++ ) {
                                if ( evaluate_aggregate( tblp, evaluationorder[i],NrColsToRecode, tblp->considered_columns) == 0 ) {
                                        //fprintf( stderr, "No solutions\n" );
                                        break;
                                }
                        }
                        if ( i < NRCOLUMNS )
                                continue;
                        nrsol++;
                        break; // <= We stop at FIRST SOLUTION now
                }
                if ( nrsol )
                        break;
        }
        printf( "FIRST SOLUTION: Seems we have %d solutions extracting %d columns from EBR and using %d-input LUTs instead\n", nrsol, NrColsToRecode, FUNCTIONSIZE );
        if ( nrsol )
                give_output();
        return 0;              
}
