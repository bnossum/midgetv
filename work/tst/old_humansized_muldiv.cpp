#include <stdlib.h>
#include "Vold_humansized_muldiv.h"
#include "verilated.h"
...
//#define PMMASK ((1u << (2*W))-1)
#define PMMASKd2 ((1u << (2*W-1))-1)
...
/////////////////////////////////////////////////////////////////////////////
void toggleclock( Vold_humansized_muldiv *tb ) {
        tb->clk = 0;
        tb->eval();
        tb->clk = 1;
        tb->eval();
...
/////////////////////////////////////////////////////////////////////////////
void initregs( Vold_humansized_muldiv *tb, int a) {
        tb->op = OP_LOAD;
        tb->Di = a;
        toggleclock(tb);
}
...
/////////////////////////////////////////////////////////////////////////////
void addcycle( Vold_humansized_muldiv *tb, int multype, int b, int ci) {
        tb->op = multype;
        tb->Di = b;
        tb->ci = ci;
        toggleclock(tb);
...
/////////////////////////////////////////////////////////////////////////////
void shiftcycle( Vold_humansized_muldiv *tb, int shifttype) {
        tb->op = shifttype;
        tb->Di = WMASK;
        toggleclock(tb);
}
...
/////////////////////////////////////////////////////////////////////////////
void test_div( Vold_humansized_muldiv *tb, int operation, const char *msg ) {
        int32_t a,b;
        int i,facitA = 0;
        int addop;
        int shiftop;
...
/////////////////////////////////////////////////////////////////////////////
void test_mul( Vold_humansized_muldiv *tb, int operation, const char *msg ) {
        int32_t a,b,i,facit;
        int addop;
        int shiftop;
        ...
        for ( a = 0; a <= WMASK; a += STEP ) {
                ...
                for ( b = 0; b <= WMASK; b += STEP ) {
                        ...
                        for ( i = 0; i < W-1; i++ ) {
                                ...
                                PRI( "  %s:%d,0x%4.4x ", willadd ? "add " : "pass", tb->dbg_rF, tb->PM );
                                shiftcycle(tb,shiftop);
                                PRI( "srl:%d,0x%4.4x\n", tb->dbg_rF, tb->PM );
                        }
                        switch ( operation ) {
...
int main(int argc, char **argv) {
	...
	Vold_humansized_muldiv *tb = new Vold_humansized_muldiv;	
